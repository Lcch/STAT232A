...
void getHistogram(const double *filter, const int width, const int height, const double min_f, const double max_f, const int *image, int *response, const int num_bins, double *filtered)
{
	int i;
	int j;
	int k;
	...
	int width_index;
	int height_index;
	double mult = (double)num_bins / (max_f - min_f);
	...
	for (i = 0; i < im_width * im_height; i++)
	{
		...
		index = int(mult * (filtered[i] - min_f) + EPS);
		if (index > num_bins) index = num_bins;
		if (index < 0) index = 0;
...
void Julesz(const double **filters, const int *width, const int *height, const double *min_f, const double *max_f, const int num_filters, const int num_bins, const double **orig_response, int *synthesized, double *final_response)
{
	time_t start;
	time_t end;
	...
	{	
		...
		getHistogram(filters[i], width[i], height[i], min_f[i], max_f[i], synthesized, syn_response, num_bins, syn_filtered[i]);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])                
{
	...
	min_f = (double *)mxGetPr(prhs[4]);
	max_f = (double *)mxGetPr(prhs[5]);
	...
	srand(time(NULL));
	for (i = 0; i < im_width * im_height; i++)
		synthesized[i] = rand() % (MAX_INTENSITY + 1);		// first, it's white noise

	Julesz(filters, width, height, min_f, max_f, num_filters, num_bins, orig_response, synthesized, syn_response);
	free(orig_response);
	free(filters);
}
